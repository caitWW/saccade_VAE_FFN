...
for epoch in trange(start_epoch, args.nepoch, leave=False):
    ...
    for i, (images, ids) in enumerate(tqdm(train_loader, leave=False)):
        ...
        with torch.cuda.amp.autocast():
            recon_img, mu, log_var = vae_net(images, ids)
        ...
        latent_vecs = mu.detach().cpu().numpy()
                 
        for i in range(len(ids)):
            image_id = ids[i]
            latent_vec_dict[image_id] = latent_vecs[i].tolist()
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                recon_img, mu, log_var = vae_net(test_images.to(device), image_ids)
                if (epoch == 4): 
                    latent_vecs = mu.detach().cpu().numpy()
            ...
            with open('/scratch/gpfs/qw3971/cnn-vae-old/rotated/new_rotated.json', 'w') as json_file:
                 json.dump(latent_vec_dict, json_file)